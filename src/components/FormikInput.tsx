...
export default function FormikInput({className, name, type,label,placeholder}: {className?: string, name: string, type: string, label: string, placeholder: string}) {
    ...
    if(type === "select" && name === 'bussinessType'){
      MyField=<Field id={name} as={type} title={label} aria-label={label} name={name} className={`bg-white text-gray-700 placeholder:text-gray-400 border-2 border-blue-500 focus:outline-none focus:bg-yellow-400  rounded-md px-1 `+ className}>
