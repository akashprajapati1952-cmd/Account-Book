...
import { addCustomer,onCustomerLoading,searchCustomer} from "../reducers/customerSlice"
...
function CustomerList({
  ...
  const displayCustomers =(!loading && search) ? Object.values(searchResults) : customers;
  ...
  return ( <main className="h-full w-full relative ">
    ...
    { displayCustomers.length !== 0 ? <section className="flex flex-col items-center p-2 gap-2">
      {displayCustomers.map((c: any)=><Custommer key={c.name} customer={c} total={c.totalTake- c.totalGive}/>)}
